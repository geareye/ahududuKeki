...
RpiBridge::RpiBridge(unsigned nId, unsigned nSerialNum, std::string sName)
...
int RpiBridge::SetGpioValue(unsigned nPortId, unsigned nPortValue)
{
	gpio = {GPIO_BASE};
	...
	if (nPortValue == PIN_VALUE_ON)
		GPIO_SET = 1 << nPortId;
	if (nPortValue == PIN_VALUE_OFF)
                GPIO_CLR = 1 << nPortId;
...
int RpiBridge::GetGpioValue(unsigned nPortId, unsigned *pnPortValue)
{
	*pnPortValue = GPIO_READ(nPortId);
}
